...
- ==Механизм версионирования данных в RAG для возможности отката изменений. Механизм реализовать с помощью формы в Pyrus. Продумать структуру формы.==
...
==На первом этапе лучше использовать форму или несколько форм в Pyrus, справочники Pyrus, которые будут служить хранилищем метаданных, логов обработки тикетов, результатов категоризации и истории обновлений RAG. Это позволит отслеживать метрики эффективности бота и анализировать качество ответов.==
...
Когда пользователь создает новое обращение в Pyrus, система Pyrus отправляет webhook на адрес бота-помощника. Бот получает базовую информацию о задаче, включая идентификатор тикета, и делает запрос к API Pyrus для получения полного контекста задачи - описания проблемы, приложенных файлов, заполненных полей формы и все комментарии, за исключением комментариев от ботов.
...
==Все действия логируются в формах\задачах, справочниках Pyrus: время обращения, категория, найденная информация в RAG, уровень уверенности AI, факт использования ответа бота сотрудником (полностью, частично, не использован).==
...
Весь процесс ETL логируется в ~~PostgreSQL~~ в формы\задачи, справочники Pyrus: количество обработанных документов из каждого источника, количество найденных и замененных персональных данных, количество новых записей в RAG, время выполнения каждого этапа. В случае ошибок отправляются уведомления в корпоративный мессенджер команде поддержки.
...
Начать разработку предлагается с минимально жизнеспособного продукта, который включает базовый функционал обработки новых тикетов и простой ETL из одного источника (например, только тикеты Pyrus). Это позволит быстро проверить гипотезу и получить обратную связь от команды поддержки.
